...
sidebarTitle: "Posts"
description: "Use groniz posts create, list, delete, and connect to schedule content across 32+ social channels, post threads, and link posts to their published release IDs."
---
...
You can filter by date range — run `groniz posts list --help` to see available flags.
...
Once you've identified the correct release ID from the output above, use `posts connect` to link the post:

```bash
groniz posts connect
```

Follow the interactive prompt (or supply flags — run `groniz posts connect --help`) to complete the connection. After connecting, analytics commands will return data for that post.
