---
title: "Upload media before posting with groniz upload"
sidebarTitle: "Media Upload"
description: "Upload images and video with groniz upload before referencing them in posts. Raw paths and external URLs are rejected — only uploaded .path values are accepted."
---
...
Groniz also supports uploading media directly from a remote URL, which saves you from downloading the file locally first:

```bash
groniz upload "https://example.com/source-image.jpg"
```

The response format is identical — you'll receive an `id` and a `path` to use in your post. The same rule applies: use the returned `.path`, not the original source URL.
