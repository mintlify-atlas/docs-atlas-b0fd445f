---
title: "Use Groniz with Claude Code: skill and MCP setup"
sidebarTitle: "Claude Code"
description: "Install the Groniz skill for Claude Code in one command, or connect via MCP for tool-based access. Both methods let Claude schedule posts across 32+ channels."
---
...
Alternatively, export your key in your shell profile so it is always available:

```bash
export GRONIZ_API_KEY=your_api_key_here
```

Add that line to `~/.zshrc`, `~/.bashrc`, or your preferred shell profile so it persists across terminal sessions and agent restarts.
